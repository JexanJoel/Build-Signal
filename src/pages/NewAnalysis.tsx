...
export default function NewAnalysis() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f1f5f9", fontFamily: "var(--font)" }}>
      ...
      <nav style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)", borderBottom: "1px solid #334155" }}
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-10 h-14 sm:h-16 flex items-center justify-between">
          ...
          <button onClick={() => navigate("/dashboard")}
            className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-semibold px-3 sm:px-4 py-2 rounded-xl transition"
      ...
      {/* Page wrapper — full width with generous padding */}
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-10 py-8 sm:py-12 lg:py-14">

        {/* Header */}
        <div className="mb-8 sm:mb-10 fade-up text-center">
          <div className="text-5xl sm:text-6xl mb-3 sm:mb-4">🔬</div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-gray-900 tracking-tight mb-2 sm:mb-3">Reality check your idea</h1>
          <p className="text-sm sm:text-base text-gray-500 leading-relaxed max-w-md mx-auto">Be specific — the more detail you give, the sharper the analysis.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">

          {/* Desktop two-column layout: Core info + Target side by side */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
            ...
            <div className="rounded-2xl border-2 overflow-hidden fade-up"
              ...
              <div className="px-5 sm:px-6 pt-5 pb-2 flex items-center gap-2">
              ...
              <div className="px-5 sm:px-6 pb-5 sm:pb-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea name="description" required value={form.description} onChange={handleChange}
                    rows={6} placeholder="What is it? How does it work? What makes it different from existing solutions?"
              ...
              <div className="px-5 sm:px-6 pb-5 sm:pb-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea name="problemSolved" required value={form.problemSolved} onChange={handleChange}
                    rows={4} placeholder="What pain point are you solving? Why does this matter to your user?"
                    className="input-glow w-full border-2 border-white bg-white text-gray-900 rounded-xl px-4 py-3 text-sm placeholder-gray-300 resize-none shadow-sm" />
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border-2 border-gray-100 p-5 sm:p-6 fade-up" style={{ animationDelay: "0.15s" }}>
              ...
              {modes.map(m => {
                ...
                return (
                  <button key={m.value} type="button"
                    ...
                    className="relative p-4 sm:p-5 rounded-xl border-2 text-left transition-all duration-200 card-lift"
                    ...
                    <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">{m.emoji}</div>
                    <div className="text-xs sm:text-sm font-black text-gray-900 mb-0.5 sm:mb-1">{m.label}</div>
                    <div className="text-xs text-gray-400 leading-snug hidden sm:block">{m.desc}</div>
                    {active && <div className="absolute top-3 right-3 w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full pulse-dot" style={{ background: m.dotColor }} />}
                  </button>
                )
              })}
            </div>
          ...
          <button type="submit" disabled={loading || !filled}
            className="btn-shine w-full flex items-center justify-center gap-3 text-white font-black py-3.5 sm:py-4 rounded-xl transition-all disabled:opacity-40 text-sm sm:text-base fade-up"
            ...
            {loading ? <><Loader2 size={16} className="animate-spin" /> Analysing your idea...</> : <>⚡ Run Reality Check</>}
          </button>

          <p className="text-center text-xs text-gray-400 fade-up pb-4 sm:pb-0" style={{ animationDelay: "0.25s" }}>
